...
## 함수형 컴포넌트 처리 단계
함수형 컴포넌트 기반의 react는 ```렌더링(Rendering)``` -> ```마운트(Mount)``` -> ```업데이트(Update)``` -> ```언마운트(Unmount)``` 의 4단계의 처리 과정을 거친다.

### 렌더링 단계
- 컴포넌트 호출(화면 전환 등)
- JSX 부분이 처리되어 View를 생성(가상 DOM)
- 이전 가상 DOM과 비교하여 변경된 부분을 찾음

이 단계에서는 아직 브라우저 화면에 아무런 변화가 나타나지 않는다. 

### 마운트 단계
- 최초 화면을 생성
- 브라우저에 생성된 화면을 출력

이 단계에서 화면에 컴포넌트가 보이게 된다.

### 업데이트 단계
- 화면에 변화가 발생(사용자의 입력 등)하면 실행되는 단계(state, props 등이 변화할 때)
- 컴포넌트가 (변화된 내용을 포함하여) 다시 렌더링됨
- 새로운 JSX로 가상 DOM 생성
- 변경된 부분만 실제 DOM에 반영(화면의 부분 변화가 발생)

### 언마운트 단계
- 컴포넌트가 화면에서 사라질 때(다른 화면으로의 전환)
- 컴포넌트 삭제 등 메모리 작업 실행

이후 다른 화면용 컴포넌트가 위와 같은 단계로 처리되어 페이지 전환이 이루어 진다.
